...
from CHOMP import PDB, NewSystem, RotamerLibrary, RotamerGenerator, FasterPacker, SimulatedAnnealingPacker, ResidueGenerator, MonteCarloPacker
...
#coreAA = 'AVLIMFW'
coreAA = 'AVLIFW'
bounAA = 'ASTCVLIFYWNQK'

core = [284,291,312,319,326,280,287,294,315,322,329]
#boun = [281,288,295,316,323,329,283,290,318,325]
boun = [281,288,295,316,330,283,290,318,325]
...
for resid in core:
    ...
    dp.AddAA(resid,coreAA)
    dp.AddAA(resid+58,coreAA)

for resid in boun:
    print 'resid ', resid
    dp.AddAA(resid,bounAA)
    dp.AddAA(resid+58,bounAA)
...
for rotidI,(resI,aaI,chiI) in rotdetail.items():
    for rotidJ,(resJ,aaJ,chiJ) in rotdetail.items():
        ...
        if resJ != resI + 58: continue  ## only apply sym benefit if residue matches
...
for i in range(0,10):
    ...
    score = energy_function(designedS)
    print 'E =', score
    open('designed_mc'+str(i)+'.pdb','w').write(str(designedS))
    open('designed_mc'+str(i)+'.seq','w').write(designedS.GetSequence())
